...
LAXVERIFY_TARGETS=$(addsuffix -verlax,$(VERIFY_FILES) $(NOVERIFY_FILES))
...
%-verlax: .cache.lax/gen .cache.lax/%.checked.lax
	@true
...
.cache.lax/%.checked.lax: %
	...
	@$(FSTAR) --cache_dir .cache.lax --lax --cache_checked_modules $< >/dev/null
